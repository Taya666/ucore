...
default_free_pages(struct Page *base, size_t n) {
    ...
    while (le != &free_list) { //try to merge
    ...
    list_entry_t *le_b = &free_list;
    while (le != &free_list) { //insert before the first bigger than base
        p = le2page(le, page_link);
        le = list_next(le);
        if( PADDR((uintptr_t)p) > PADDR((uintptr_t)base) ){
            le_b = &(p->page_link);
            break;
        }
    }
    list_add_before(le_b, &(base->page_link));
}
...
default_check(void) {
    ...
    while ((le = list_next(le)) != &free_list) {
        ...
        count ++, total += p->property; //遍历free block count记录多少个free block，total记录总的空闲页
    }
