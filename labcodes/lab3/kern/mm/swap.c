...
swap_out(struct mm_struct *mm, int n, int in_tick)
{
     ...
     for (i = 0; i != n; ++ i)
     {
          ...
          cprintf("SWAP: choose victim page 0x%08x link %x \n", page, &(page->pra_page_link));
...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
     ...
     cprintf("swap_in: load disk swap entry %d with swap_page in vadr 0x%x pra_vaddr %x\n", (*ptep)>>8, addr, result->pra_vaddr);
     *ptr_result=result;
     return 0;
}
...
check_swap(void)
{
    //backup mem env
     ...
     //0x1000 - 0x6000 4kb-24kb  5个虚拟页 [4kb,20kb] 0x1000 0x5000
     ...
     check_content_set(); //触发4次page fault，将[0x1000, 0x4000]都建立映射 ， 不回触发swap
     assert( nr_free == 0);         
